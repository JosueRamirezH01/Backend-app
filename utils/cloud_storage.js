...
const bucket = storage.bucket("gs://medico-app-7fe60.appspot.com/");

/**
 * Subir el archivo a Firebase Storage
 * @param {File} file objeto que sera almacenado en Firebase Storage
 */
module.exports = (file, pathImage, deletePathImage, pathPdf, deletePathPdf, pathPdfM) => {
    return new Promise((resolve, reject) => {
       ...
       console.log('delete path', deletePathImage)
        ...
        if (pathPdf) {
            if (pathPdf != null || pathPdf != undefined) {

                let fileUpload = bucket.file(`${pathPdf}`);
                let stream = fileUpload.createWriteStream();
                const blobStream = stream.pipe(fileUpload.createWriteStream({
                    metadata: {
                        contentType: 'application/pdf', // Establecer el tipo de contenido a PDF
                        metadata: {
                            firebaseStorageDownloadTokens: uuid,
                        }
                    },
                ...
                blobStream.on('finish', () => {
                    // URL pública para acceder al archivo PDF a través de HTTP.
                    const url = format(`https://firebasestorage.googleapis.com/v0/b/${bucket.name}/o/${fileUpload.name}?alt=media&token=${uuid}`);
                    console.log('URL DE CLOUD STORAGE ', url);
                    resolve(url);
                });
        ...
        if (pathPdfM) {
            if (pathPdfM != null || pathPdfM != undefined) {

                let fileUpload = bucket.file(`${pathPdfM}`);
                let stream = fileUpload.createWriteStream();
                const blobStream = stream.pipe(fileUpload.createWriteStream({
                    metadata: {
                        contentType: 'application/pdf', // Establecer el tipo de contenido a PDF
